...
library LibReputation {
    int32 internal constant MIN_SCORE = -10000;
    int32 internal constant MAX_SCORE = 10000;
    int32 internal constant CANCELLATION_PENALTY = -1;
    int32 internal constant COMPLETION_REWARD = 1;

    event LabReputationAdjusted(
    ...
    function recordOwnerCancellation(uint256 labId) internal returns (int32 newScore) {
    ...
    function recordInstitutionalCancellation(uint256 labId) internal returns (int32 newScore) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        LabReputation storage rep = s.labReputation[labId];
    ...
    function recordCompletion(uint256 labId) internal returns (int32 newScore) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        LabReputation storage rep = s.labReputation[labId];
    ...
    function adjustScore(uint256 labId, int32 delta, string memory reason) internal returns (int32 newScore) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        LabReputation storage rep = s.labReputation[labId];
    ...
    function setScore(uint256 labId, int32 newScore, string memory reason) internal {
        AppStorage storage s = LibAppStorage.diamondStorage();
        LabReputation storage rep = s.labReputation[labId];
    ...
    function _applyDelta(LabReputation storage rep, int32 delta) private returns (int32 newScore) {
    ...
    function _clampScore(int32 score) private pure returns (int32) {
