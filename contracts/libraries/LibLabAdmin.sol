...
library LibLabAdmin {
    ...
    function _hasActiveBookings(
        ...
        AppStorage storage s = _s();
        return s.labPendingReservationCount[_labId] > 0 || s.labActiveReservationCount[_labId] > 0
            || s.providerReceivableAccrued[_labId] > 0 || s.providerSettlementQueue[_labId] > 0
            || s.providerReceivableInvoiced[_labId] > 0 || s.providerReceivableApproved[_labId] > 0
            || s.providerReceivableDisputed[_labId] > 0;
    }
