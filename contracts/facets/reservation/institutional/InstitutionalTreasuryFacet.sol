...
contract InstitutionalTreasuryFacet is ReentrancyGuardTransient {
    ...
    function _onlyAuthorizedBackendOrInternal(
        address institution
    ) internal view {
    ...
    function _calculatePeriodStart(
        uint256 timestamp,
    ...
    function setInstitutionalUserLimit(
        uint256 limit
    ) external onlyInstitutionCaller {
        AppStorage storage s = LibAppStorage.diamondStorage();
    ...
    function getInstitutionalUserLimit(
        address institution
    ) external view returns (uint256) {
        ...
        )
    {
        ...
        remainingAllowance = spendingLimit > currentPeriodSpent ? spendingLimit - currentPeriodSpent : 0;
