...
library LibProviderReceivable {
    ...
    function accrueReceivable(uint256 labId, uint256 amount, bytes32 reservationKey) internal {
    ...
    function updateAccruedTimestamp(uint256 labId, uint256 timestamp) internal {
        AppStorage storage s = LibAppStorage.diamondStorage();
    ...
    function hasUnsettledReceivable(uint256 labId) internal view returns (bool) {
        AppStorage storage s = LibAppStorage.diamondStorage();
        return s.providerReceivableAccrued[labId] > 0
            || s.providerSettlementQueue[labId] > 0
            || s.providerReceivableInvoiced[labId] > 0
            || s.providerReceivableApproved[labId] > 0
