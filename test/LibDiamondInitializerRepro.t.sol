...
contract LibDiamondInitializerRepro is Test {
    ...
    function test_repro_initializeDiamondCut_smallCut() public {
        ...
        cut[0] = IDiamond.FacetCut({ facetAddress: address(mockFacet), action: IDiamond.FacetCutAction.Add, functionSelectors: sel0 });

        bytes4[] memory sel1 = new bytes4[](1);
        sel1[0] = bytes4(0x8be0079c);
        cut[1] = IDiamond.FacetCut({ facetAddress: address(0x1234), action: IDiamond.FacetCutAction.Add, functionSelectors: sel1 });

        bytes4[] memory sel2 = new bytes4[](3);
        sel2[0] = bytes4(0x94499827);
        sel2[1] = bytes4(0x0e4562a1);
        sel2[2] = bytes4(0x057c384a);
        cut[2] = IDiamond.FacetCut({ facetAddress: address(0xBEEF), action: IDiamond.FacetCutAction.Add, functionSelectors: sel2 });
