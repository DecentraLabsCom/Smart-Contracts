...
/**
 * @dev Defines the initial amount of LAB tokens assigned to a new entity.
 * This constant is used to set the starting balance for LAB tokens.
 */
...
contract ProviderFacet is AccessControlUpgradeable {
      ...
      * @param account The address of the provider being added.
      * @param name The name of the provider.
      * @param email The email address of the provider.
      * @param country The country of the provider.
      */
     event ProviderAdded(address indexed account, string name, string email, string country);

     /**
      * @dev Emitted when a provider is removed.
      * @param account The address of the provider that was removed.
      */
     event ProviderRemoved(address indexed account);

     /**
      * @dev Emitted when a provider's information is updated.
      * @param account The address of the provider.
      * @param name The name of the provider.
      * @param email The email address of the provider.
      * @param country The country of the provider.
      */
     event ProviderUpdated(address indexed account, string name, string email, string country);
      ...
      *
      * Reverts:
      * - If the caller does not have the `DEFAULT_ADMIN_ROLE`, the transaction is reverted
      *   with the error message: "Only the default admin can perform this action".
      */
     ...
     * 
     * @custom:emits Emits a {RoleGranted} event if the role is successfully granted.
     */
      ...
      * @custom:modifier `defaultAdminRole` - Only callable by accounts with the default admin role.
      * @custom:reverts Reverts with "Provider already exists" if the account already has the `PROVIDER_ROLE`.
      * @custom:emits Emits a `ProviderAdded` event with the provider's details.
      */
      ...
      *      The function emits a `ProviderRemoved` event upon successful removal.
      * @param _provider The address of the provider to be removed.
      * @custom:warning Ensure that the provider being removed does not have associated Labs,
      *                 as this function does not handle such cases. Additional logic may be required.
      * @custom:emit Emits a `ProviderRemoved` event upon successful removal of the provider.
      */
     function removeProvider(address _provider) external onlyRole(DEFAULT_ADMIN_ROLE)  {
          ...
          // TODO: What if the provider has Labs?
     }
     ...
     *
     * Emits an {ProviderUpdated} event.
     */
